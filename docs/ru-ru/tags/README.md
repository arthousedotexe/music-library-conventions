...
Начнем с того, какие вообще теги я использую с учетом огромного их количества.

**Примечание:** названия тегов в этом разделе приведены как обобщенные понятия. Конкретные имена полей зависят от формата метаданных.  
Например, `Album Artist` соответствует полю `ALBUMARTIST` в **Vorbis Comment (FLAC, Opus)**, атому `aART` в **iTunes MP4 (ALAC, AAC)**, и `TPE2` фрейму в **ID3v2 (MP3)**.
...
- `Artist` - исполнитель или исполнители трека;  
  **Примечание:** данный тег является мультитегом (поддерживает несколько значений одновременно).  
  Я не объединяю артистов в одну строку через разделители вроде `feat.`, `&`, `,` или `;`.  
  **Пример в Vorbis Comment:** `ARTIST=Lana Del Rey; ARTIST=Zella Day; ARTIST=Weyes Blood`, `ARTIST=Zachz Winner; ARTIST=Frozy; ARTIST=joyful`, т.е. для трех артистов используется три отдельных значения тега.

- `Date` - дата выхода конкретного релиза;  
  **Формат:** `YYYY-MM-DD` (или `YYYY`, если точный день и месяц неизвестны)  
  **Особенность ID3v2.3**: спецификация [ID3v2.3](https://id3.org/id3v2.3.0) ограничивает фрейм `TYER` только годом (`YYYY`), но теггеры записывают полную дату напрямую в `TYER` без разделения с фреймом `TDAT`, как предписывает стандарт.
...
- `Cover` - встроенная обложка трека. Подробнее об этом читайте в разделе [Обложки, сканы, буклеты](/ru-ru/covers-and-booklets/?id=Встроенные-обложки).

Типичный файл с основными тегами будет выглядеть следующим образом:
![Пример №1](../../images/tags-example1.png)
...
Для последующих тегов я также указываю, могут ли они отображаться и использоваться для сортировки в музыкальных плеерах, которые я использую.
...
- `Lyrics` - синхронизированные или несинхронизированные тексты песен;  
  **Отображение:** Poweramp (+), foobar2000 (+)  
  **Примечание:** вместо этого тега я использую `.lrc` файлы, подробнее об этом читайте в разделе [Тексты песен](/ru-ru/lyrics/).
...
![Пример №2](../../images/tags-example2.png)
...
- `Album Artist Sort` - тег, который отвечает за правило сортировки исполнителя альбома, например, по фамилии или с учетом артикля;  
  Подробнее с правилами сортировки имен артистов можно ознакомиться в [документации MusicBrainz](https://musicbrainz.org/doc/Style/Artist/Sort_Name).  
...
- `Grouping` - тег, который обеспечивает дополнительный уровень группировки треков внутри релиза;  
  **Пример:**  
  В [этом релизе на Spotify](https://open.spotify.com/album/6eOuqhCfrTPp1H0YbQ9PmL) объединены две симфонии: №5 и №7.  
  ![Пример с тегом Grouping](../../images/tags-grouping.png)
  Если в тег `Grouping` для треков с 1 по 4 прописать `Symphony No. 5 in C Minor, Op. 67`, а для остальных — `Symphony No. 7 in A Major, Op. 92`, музыкальный плеер сможет визуально разделить альбом на две секции (если поддерживает отображение группировок).  
...
- `MusicBrainz IDs` - уникальные идентификаторы из базы данных MusicBrainz;  
  ...
  **Отображение:** Poweramp (-), foobar2000 (+)  
  **Сортировка по данному тегу:** Poweramp (-), foobar2000 (+)  
  **Примечание:** дополнительную информацию читайте в документации MusicBrainz по [идентификаторам MBID](https://musicbrainz.org/doc/MusicBrainz_Identifier), [базовым тегам](https://picard-docs.musicbrainz.org/en/latest/variables/tags_basic.html) и [сопоставлению метаданных](https://picard-docs.musicbrainz.org/en/latest/appendices/tag_mapping.html).  
  Также эти теги полезны для связи с медиа-серверами ([Navidrome](https://www.navidrome.org/), [Jellyfin](https://jellyfin.org/)), скробблерами ([ListenBrainz](https://listenbrainz.org/), self-hosted скробберы, например, [Koito](https://github.com/gabehf/Koito)), и с базой данных [MusicBrainz](https://musicbrainz.org/).
...
- `ReplayGain Tags` - теги, которые отвечают за ReplayGain;  
  **А именно:**  
  ...
  - `ReplayGain Track Peak` - тег, который содержит максимальный пиковый уровень сигнала внутри одного трека. Позволяет музыкальному плееру снизить громкость и предотвратить клиппинг, если расчитанное усиление (`ReplayGain Track Gain`) приводит к превышению цифрового порога 0 dBFS;
  ...
  **Примечание:** подробнее о ReplayGain читайте в [статье на Википедии](https://ru.wikipedia.org/wiki/ReplayGain) и в [технической документации Hydrogenaudio](https://wiki.hydrogenaudio.org/index.php/ReplayGain).  

Итоговый результат будет выглядеть примерно так::

![Пример №3](../../images/tags-example3.png)
...
- Они содержат информацию, которая несет мало практической ценности, либо ее нельзя считать достоверным доказательством происхождения файла;
...
- `Artists` - мультитег, в котором хранится список нескольких артистов;  
  **Примечание:** этот тег генерируется и заполняется автоматически MusicBrainz Picard, если соответствующая информация доступна в базе данных MusicBrainz. Подробнее смотрите раздел [Базовые теги](https://picard-docs.musicbrainz.org/en/latest/variables/tags_basic.html) в документации MusicBrainz.  
...
- `Engineer` - теги, содержащие звукоинженеров, звукорежиссеров, звукооператоров;
...
- `Lyricist` — человек, который пишет тексты к песням;

- `Mixer` — человек, ответственный за сведение аудиозаписи;

- `Mood` - настроение трека;  
  **Примечание:** удачный пример классификации настроений музыки на основе модели Тэйера разобран в [материале Tufts University](https://sites.tufts.edu/eeseniordesignhandbook/2015/music-mood-classification/).  
...
- `Script` — скрипт, используемый для написания трек-листа релиза;  
  **Примечание:** под скриптом здесь подразумевается набор графических символов, используемых для письменного обозначения одного или нескольких языков. Подробнее о стандарте читайте в статье об [ISO 15924 на Википедии](https://en.wikipedia.org/wiki/ISO_15924).  
...
- `Work` - это отдельное интеллектуальное или художественное произведение, которое может быть выражено в форме одной или нескольких аудиозаписей. Произведение не обязательно должно являться музыкальным. Например, произведением может быть роман, пьеса, стихотворение или эссе, позже записанные в виде аудиокниги;  
  **Примечание:** подробнее о концепции произведений смотрите в [документации MusicBrainz (Work)](https://musicbrainz.org/doc/Work).  

- `Writer` - человек, который создает музыкальные композиции или пишет тексты к песням, или и то, и другое одновременно.
...
- ID3v2.3 (MP3):  
  На практике теггеры записывают полную дату (`YYYY-MM-DD`) напрямую во фрейм `TYER`, не используя фрейм `TDAT`.  
