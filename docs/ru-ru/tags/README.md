...
- ```Artist``` - исполнитель или исполнители трека;  
  **Примечание:** если исполнителей у треков несколько, то они пишутся через ```\\```  
  **Примеры:** ```Lana Del Rey\\Zella Day\\Weyes Blood```, ```Zachz Winner\\Frozy\\joyful```

- ```Date``` - дата выхода альбома;  
  **Формат:** YYYY-MM-DD
...
- ```Original Date``` - первоначальная дата выхода альбома;  
  **Формат:** YYYY-MM-DD  
  ...
  **Примечание:**  Если Original Date дублирует дату в Date, то тогда я его не сохраняю за ненадобностью.  
...
- ```Barcode``` - уникальный штрихкод музыкального релиза, нужен для идентификации альбома в цифровых магазинах и стриминговых сервисах.  
  **Отображение:** Poweramp (-), Foobar2000 (+)  
  **Сортировка по данному тегу:** Poweramp (-), Foobar2000 (+)
...
- ```Catalogue Number``` - уникальный код, который присваивается конкретному физическому изданию.  
  **Отображение:** Poweramp (-), Foobar2000 (+)  
  **Сортировка по данному тегу:** Poweramp (-), Foobar2000 (+)
...
- ```Compilation``` - флаг, который указывает, что трек является частью сборника разных исполнителей.  
  **Сортировка по данному тегу:** Poweramp (-), Foobar2000 (необходима настройка паттерна)

- ```ISRC``` - уникальный международный код, который присваивается конкретной аудиозаписи, нужен для идентификации альбома в цифровых магазинах и стриминговых сервисах.  
  **Отображение:** Poweramp (-), Foobar2000 (+)  
  **Сортировка по данному тегу:** Poweramp (-), Foobar2000 (+)
...
**Исключенные теги** - бесполезные/практически бесполезные теги или теги, которые обладают исключительной субъективностью.

- ```Artists``` - мультитег, используемый MusicBrainz для хранения нескольких артистов.  
...
- ```Subtitle``` - подзаголовок трека.  
  **Пример:** ```Acoustic Version```, пока в теге ```Title``` в этот момент будет находиться название без указания версии.
  Без использования этого тега название в теге ```Title``` будет выглядеть так: ```Title (Acoustic Version)``.
