...
- `Artist` - исполнитель или исполнители трека;  
  **Примечание:** если исполнителей у трека несколько, то разделителями между ними являются символы ``\\``.  
  В программе Mp3tag несколько значений отображаются через `\\`. Сам разделитель `\\` в поле тега не сохраняется.  
...
- `Grouping` - тег, который обеспечивает дополнительный уровень группировки между релизом и отдельными треками;  
  **Пример:**  
  Возьмём [данный релиз на Spotify](https://open.spotify.com/album/6eOuqhCfrTPp1H0YbQ9PmL), в нём содержатся две симфонии: номер 5 и номер 7.
