...
На деле же в **FLAC (Vorbis Comment)** за это отвечает тег ```ALBUMARTIST```, в **ALAC (iTunes MP4)** за это отвечает ```aART``` атом, а в **MP3 (ID3v2.3, ID3v2.4)** - ```TPE2``` фрейм.
...
- ```Language``` - язык, на котором звучит релиз;  
  **Примечание:** это трехсимвольный языковой код, подробнее [здесь](https://en.wikipedia.org/wiki/ISO_639-3).  
...
- ```MusicBrainz Tags``` - теги, содержащие уникальные идентификаторы из базы данных MusicBrainz;  
  ...
  **Примечание:** читать подробнее [здесь](https://musicbrainz.org/doc/MusicBrainz_Identifier), [здесь](https://picard-docs.musicbrainz.org/en/latest/variables/tags_basic.html) и [здесь](https://picard-docs.musicbrainz.org/en/latest/appendices/tag_mapping.html).  
  Также эти теги полезны для связи с каталогами (Navidrome, Plex), скробблерами (ListenBrainz, self-hosted скробберы), и с самой базой данных MusicBrainz.

- ```Performer``` - теги, которые отвечают за роли музыкантов. Например, кто является гитаристом, кто вокалистом, кто на барабанах и так далее;  
...
- ```Encoded By``` - человек, который создал/перекодировал аудиофайл, например, автор рипа;
...
- ```Subtitle``` - подзаголовок трека;  
  **Пример:** пусть в ```Subtitle``` будет лежать значение ```Акустическая версия```, тогда в теге ```Title``` будет лежать значение ```Имя трека```.  
  Без использования тега ```Subtitle``` название в теге ```Title``` будет выглядеть следующим образом: ```Имя трека (Акустическая версия)```.
...
- iTunes MP4:  
  Все теги, начинающиеся с ```----```, являются пользовательскими метаданными, остальные теги являются стандартными атомами формата.
