...
I do not use audio stream checksums for lossy formats. They are non-deterministic because compiler differences, architectures, codec implementations (for example, ```OPUS``` has fixed-point and floating-point implementations, more details [here](https://github.com/xiph/opus/blob/main/README) in the Portability notes section), decoders, conversion settings, and other variables. Checksums will be generated differently each time.

File checksums (which depend not only on audio stream but also on metadata, cover art, padding in tags, and so on) I also do not use.
